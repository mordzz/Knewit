...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return withErrorHandling(async () => {
    ...
    const { data: parent } = await supabase.from('comments').select('id').eq('id', id).maybeSingle();
    ...
    const { items: rows, nextCursor: pageCursor } = await fetchPage<CommentRow>(
      supabase.from('comments').select('*').eq('parent_comment_id', id).order('created_at', { ascending: true }),
      offset
    );
