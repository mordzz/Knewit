...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return withErrorHandling(async () => {
    ...
    const { data: comment } = await supabase.from('comments').select('share_count').eq('id', id).maybeSingle();
    if (!comment) throw notFound(`Comment ${id} not found.`);

    const shareCount = (comment.share_count ?? 0) + 1;
    const { error } = await supabase.from('comments').update({ share_count: shareCount }).eq('id', id);
