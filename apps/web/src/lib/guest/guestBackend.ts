...
/** The `{ code, message }` wire shape API errors use — same shape as
 * `apiClient.ts`'s own error body, defined locally to avoid a cycle. */
...
export function selfCheckGuestBackend(): void {
  if (selfChecked || process.env.NODE_ENV === 'production') return;
  selfChecked = true;
  const state = useGuestStore.getState();
...
export async function guestRequest<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  // Web services call this same Next.js app's `/api/*` routes; the
  // sandbox's own route table is written without that prefix.
...
function route(
  ...
  if (pathname === '/markets/trending' && method === 'GET') {
    const items = data.guestMarketList().filter((item) =>
      item.kind === 'market' ? item.market.trending : item.group.trending
    );
  ...
  if (pathname === '/feed' && method === 'GET') return paginateFeed(state, data.rawGuestFeed(state), params);
...
function createCall(state: GuestState, body: unknown): FeedItem {
  ...
  const market = state.markets[position.marketId] ?? data.marketSummaryOf(data.guestMarketDetail(position.marketId));
...
function toggleCallLike(state: GuestState, callId: string, liked: boolean) {
  ...
  const next = liked
    ...
    : { liked: false, likeCount: current.liked ? Math.max(0, current.likeCount - 1) : current.likeCount };
