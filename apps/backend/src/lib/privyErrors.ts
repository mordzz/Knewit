 ...
 * from the crypto sub-flow) — verified in the installed
 * `@privy-io/react-auth` bundle, since the public types don't expose a
 * cancel code. Kept in one place so a future SDK wording change is a
 * one-file fix; anything unrecognised is treated as a real error.
 */
