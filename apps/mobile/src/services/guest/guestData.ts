...
import { MOCK_PEOPLE, searchMockPeople } from '@/features/search/fixtures/people.mock';
import { GUEST_USER_ID } from '@/store/guest/guestStore';
import type { GuestState } from '@/store/guest/guestStore';
...
export function rawGuestUserCalls(state: GuestState, userId: string): FeedItem[] {
  ...
  return materializePages(mockUserContentTotal(), (cursor) => buildMockUserContent(userId, cursor));
}
...
export function guestCommentReplies(
  ...
  return rawGuestReplies(state, commentId, postId).map((item) => applyCommentOverlay(state, item));
}
...
export function rawGuestUserReplies(state: GuestState, userId: string): CommentItem[] {
  if (userId === 'me' || userId === GUEST_USER_ID) {
    ...
    const created = state.comments.filter(
      (comment) =>
        comment.author.id === GUEST_USER_ID && !state.deletedCommentIds.includes(comment.id)
    );
    return [...created, ...fixture];
  }
  return materializePages(mockUserRepliesTotal(), (cursor) => buildMockUserReplies(userId, cursor));
}
...
const LEADERBOARD_VOLUMES = [
  1_840_000, 1_275_000, 980_400, 742_100, 615_800, 402_600, 318_200, 154_900,
];
...
function buildGroup(parsed: ParsedGroup): MarketGroupSummary | null {
  ...
  return (
    groups[parsed.index] ?? buildMockGroups(4, parsed.category)[0] ?? buildMockGroups(4)[0] ?? null
  );
}
...
function childSummary(
  ...
  return {
    ...
    noPrice: row.noPrice,
    volume:
      group.volume == null ? null : Math.round(group.volume / Math.max(1, group.outcomes.length)),
...
export function guestEventActivity(state: GuestState, eventId: string): FeedItem[] {
  ...
  const created = state.createdCalls.filter(
    (item) =>
      item.market != null && childIds.has(item.market.id) && !state.deletedCallIds.includes(item.id)
  );
...
export function guestEventHolders(
  ...
  const markets = marketId
    ? event.markets.filter((market) => market.id === marketId)
    : event.markets;
