...
/**
 * Crypto deposit sheet state. While `active`, re-reads the bridge
 * addresses, supported assets and transfer status every 10s. The bridge
 * delivers pUSD itself, so there is nothing to convert: a completed
 * transfer just refreshes the balance.
 */
export function useCryptoDeposit(active: boolean) {
  ...
  const completedAt =
    ...
    latest.createdAtMs != null &&
    info.dataUpdatedAt - latest.createdAtMs <= JUST_COMPLETED_MS
      ? latest.createdAtMs
      : null;
